...
export default function ThemeToggle() {
  function toggle() {
    ...
    applyTheme(next)
    try { localStorage.setItem(THEME_KEY, next) } catch { /* Still works without persistence. */ }
  }
  return <button type="button" onClick={toggle} className="theme-toggle" aria-label="Переключить светлую и тёмную тему" title="Переключить тему">
